...
export default function CriaPage() {
  ...
  const [qrModalZone, setQrModalZone] = useState<Zone | null>(null);
  const { toast } = useToast();

  const { data: zones = [], isLoading } = useQuery<Zone[]>({
    queryKey: ["/api/zones", { stage: "cria" }],
    queryFn: async () => {
      const res = await apiRequest("GET", "/api/zones?stage=cria");
      return res.json();
    }
  });
  ...
  const createZoneMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Zona creada",
        description: "La zona de cría ha sido creada correctamente",
      });
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground mb-2">Cría</h1>
            <p className="text-muted-foreground">Gestiona las zonas de cría para lechones y cerdos jóvenes</p>
          </div>
          <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Crear Nueva Zona de Cría</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleCreateZone} className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ej: Zona A1, Corral Norte, etc."
                    required
          ...
          {isLoading ? (
            ...
            <Card className="col-span-full">
              <CardContent className="text-center py-12">
                <p className="text-muted-foreground mb-4">No hay zonas de cría creadas</p>
        ...
        {/* QR Modal */}
        <Dialog open={!!qrModalZone} onOpenChange={() => setQrModalZone(null)}>
          <DialogContent className="max-w-md">
            <DialogHeader>
              <DialogTitle>QR para movimiento de lotes</DialogTitle>
            </DialogHeader>
            {qrModalZone && <ZoneQrDisplay zone={qrModalZone} />}
          </DialogContent>
        </Dialog>
      </div>
    </MainLayout>
