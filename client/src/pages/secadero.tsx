...
export default function SecaderoPage() {
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [newZoneName, setNewZoneName] = useState("");
  const { toast } = useToast();

  const { data: zones = [], isLoading } = useQuery<Zone[]>({
    queryKey: ["/api/zones", { stage: "secadero" }],
    queryFn: async () => {
      const res = await apiRequest("GET", "/api/zones?stage=secadero");
      return res.json();
    }
  });
  ...
  const createZoneMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Zona creada",
        description: "La zona de secadero ha sido creada correctamente",
      });
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground mb-2">Secadero</h1>
            <p className="text-muted-foreground">Gestiona las zonas de curado y secado de jamones y embutidos</p>
          </div>
          <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Crear Nueva Zona de Secadero</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleCreateZone} className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ej: Cámara 1, Bodega Norte, etc."
                    required
          ...
          {isLoading ? (
            ...
            <Card className="col-span-full">
              <CardContent className="text-center py-12">
                <p className="text-muted-foreground mb-4">No hay zonas de secadero creadas</p>
