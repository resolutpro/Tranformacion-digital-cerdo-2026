...
export default function MataderoPage() {
  ...
  const { data: zones = [], isLoading } = useQuery<Zone[]>({
    queryKey: ["/api/zones", { stage: "matadero" }],
    queryFn: async () => {
      const res = await apiRequest("GET", "/api/zones?stage=matadero");
      return res.json();
    }
  });
  ...
  const createZoneMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Zona creada",
        description: "La zona de matadero ha sido creada correctamente",
      });
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground mb-2">Matadero</h1>
            <p className="text-muted-foreground">Gestiona las zonas de procesamiento y sacrificio</p>
          </div>
          <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Crear Nueva Zona de Matadero</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleCreateZone} className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Ej: Sala de Sacrificio 1, Línea A, etc."
                    required
          ...
          {isLoading ? (
            ...
            <Card className="col-span-full">
              <CardContent className="text-center py-12">
                <p className="text-muted-foreground mb-4">No hay zonas de matadero creadas</p>
