                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-medium text-orange-900">
                          {dashboardData.loteCounts.unassigned} lotes sin
                          asignar
                        </p>
                        <p className="text-sm text-orange-700">
                          Estos lotes necesitan ser asignados a una zona para
                          comenzar el proceso
                        </p>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-8">
              {Object.entries(dashboardData.loteCounts)
                ...
                .map(([stage, count]) => {
                  ...
                  const animalCount =
                    dashboardData.animalCounts?.[
                      stage as keyof typeof dashboardData.animalCounts
                    ] || 0;
                          ...
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <div className="text-right">
                              ...
                              <span
                                ...
                                data-testid={`animal-count-${stage}`}
                              >
              ...
              <Card
                className={
                  dashboardData.loteCounts.finished > 0
                    ? "border-green-200 bg-green-50/50"
                    : ""
                }
                data-testid="card-finished"
              >
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-3">
                ...
                {dashboardData.zoneActivity.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {dashboardData.zoneActivity.map((activity) => (
                      // CORRECCIÓN: Se cambia el href para que coincida con la ruta /zona/:id de App.tsx
                      <Link
                        key={activity.zone.id}
                        href={`/zona/${activity.zone.id}`}
                      >
                        <Card
                          className="cursor-pointer hover:bg-muted/50 transition-colors"
                          data-testid={`zone-activity-${activity.zone.id}`}
                        >
                          <CardContent className="p-4">
                            <div className="flex items-center justify-between mb-3">
                              <h3
                                className="font-medium text-foreground"
                                data-testid={`zone-name-${activity.zone.id}`}
                              >
                                {activity.zone.name}
                              </h3>
                              <span
                                className="text-xs text-muted-foreground"
                                data-testid={`zone-last-activity-${activity.zone.id}`}
                              >
                            ...
                            <div className="flex gap-2 flex-wrap">
                              {activity.readings.slice(0, 3).map((reading) => {
                                ...
                                const unit =
                                  reading.sensor.unit ||
                                  (reading.sensor.sensorType === "temperature"
                                    ? "°C"
                                    : reading.sensor.sensorType === "humidity"
                                      ? "%"
                                      : "");
